...
.FUNCTIONALITY
   PowerShell v3
.VERSION
   Patrick Michael Flanagan 26/07/2019 v1.3
#>
...
## Deletes the contents of C:\Temp folder.
...
## Deletes the contents of the Windows CCM Cache folder.
Get-ChildItem "C:\Windows\ccmcache\*" -Recurse -Force -Verbose -ErrorAction SilentlyContinue |
Where-Object { ($_.CreationTime -lt $(Get-Date).AddDays(-$DaysToDelete)) } |
remove-item -force -Verbose -recurse -ErrorAction SilentlyContinue
## The Contents of Windows CCM Cache have been removed successfully!
...
## Remove all files and folders in user's Windows Working Directory. 
Get-ChildItem "C:\users\*\Application Data\Local\Microsoft\Windows\WER\*" `
-Recurse -Force -Verbose -ErrorAction SilentlyContinue |
Where-Object {($_.CreationTime -le $(Get-Date).AddDays(-$DaysToDelete))} |
remove-item -force -recurse -ErrorAction SilentlyContinue
## All Windows Wer Files have been removed successfully!

## Remove all .OST files. 
##Get-ChildItem "C:\Users\*\AppData\Local\Microsoft\Outlook\*.ost" `
##-Recurse -Force -Verbose -ErrorAction SilentlyContinue |
##Where-Object {($_.CreationTime -le $(Get-Date).AddDays(-$DaysToDelete))} |
##remove-item -force -recurse -ErrorAction SilentlyContinue
## All .OST Files have been removed successfully!
